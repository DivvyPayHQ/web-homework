defmodule Homework.UsersTest do
  ...
  describe "users" do
    ...
    setup do
      ...
      {:ok, company2} =
        Companies.create_company(%{name: "some other company name", credit_line: 543.21})

      valid_attrs = %{dob: "some dob",first_name: "some first_name",last_name: "some last_name",company_id: company1.id}

      update_attrs = %{dob: "some updated dob",first_name: "some updated first_name",last_name: "some updated last_name",company_id: company2.id}
    ...
    test "get_user_by_fuzzy/2 returns a user that partially or totally matches this first/last name", %{valid_attrs: valid_attrs} do
      user = user_fixture(valid_attrs)
