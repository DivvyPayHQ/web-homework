defmodule Homework.UsersTest do
  ...
  describe "users" do
    ...
    setup do
     {:ok, company1} = Companies.create_company(%{name: "some company", credit_line: 99, available_credit: 100})

     {:ok, company2} = Companies.create_company(%{name: "some company2", credit_line: 199, available_credit: 200})
    ...
    invalid_attrs = %{dob: nil, first_name: nil, last_name: nil, company_id: company1.id}
     {:ok,
       %{
    ...
    test "change_user/1 returns a user changeset", %{updated_user: updated_user} do
      user = user_fixture(updated_user)
